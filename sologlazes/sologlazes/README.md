...
## Admin ampliado + checkout real (última tanda)

- **`/admin/productos/[slug]/editar`** — edición real de productos (nombre, serie, descripciones, precio, activo/en stock)
- **`/admin/pedidos/[id]`** — vista completa del pedido: productos, datos de contacto (nombre/tel/email — incluso para compras como invitado, guardados en `customerComment`), dirección, método de envío elegido, estado de pago, gestión de estado/tracking
- **`/admin/estadisticas`** — sumado: más vendidos (top 5) y pedidos por estado
- **Checkout con 3 métodos de envío reales**: Mensajería propia en CABA, Andreani, Correo Argentino
- **Pedido rápido por WhatsApp** — botón alternativo en `/carrito`: el cliente solo deja su número, se crea un pedido real en la base (status PENDING, sin dirección todavía) y se abre WhatsApp con el detalle prellenado para que el vendedor cierre el pedido manualmente
- Se sacó la sección **Galería** (público y home) — no existía contenido real en el sitio original; el panel de moderación en `/admin/galeria` se mantiene para cuando haya fotos reales que cargar

**Limitación conocida:** la edición de pedidos en admin cubre estado/tracking, no edición de línea de ítems (agregar/quitar productos de un pedido ya creado) — eso queda para una iteración futura si hace falta.
